...
export const Route = createFileRoute(
	...
	loader: () => {
		return { crumb: "Adult Mosquito Complaint" };
	},
});
		...
		<div className="max-w-2xl space-y-6">
			<nav className="flex items-center justify-between rounded-lg border bg-card p-4">
				...
				<Button asChild size="sm" variant="outline">
					<Link
						...
						to="/service-requests/adult-mosquito/$requestId/edit"
					>
						<Edit />
						Edit
					</Link>
			...
			<article className="space-y-4">
				<div className="flex items-baseline gap-2">
					<h2 className="font-bold text-2xl">Adult Mosquito Complaint</h2>
				...
				<div className="rounded-lg border p-4">
					<h3 className="mb-2 font-semibold">Time of Day</h3>
					<div className="flex gap-2">
						{request.is_daytime && <Badge variant="outline">Daytime</Badge>}
						{request.is_dusk_dawn && <Badge variant="outline">Dusk/Dawn</Badge>}
						{request.is_nighttime && <Badge variant="outline">Nighttime</Badge>}
					</div>
				</div>

				<div className="rounded-lg border p-4">
					<h3 className="mb-2 font-semibold">Location of Activity</h3>
					<div className="flex gap-2">
						{request.is_front_of_property && (
							<Badge variant="outline">Front of Property</Badge>
						)}
						{request.is_rear_of_property && (
							<Badge variant="outline">Rear of Property</Badge>
						)}
						{request.is_general_vicinity && (
							<Badge variant="outline">General Vicinity</Badge>
						)}
					</div>
				</div>

				<div className="rounded-lg border p-4">
					<h3 className="mb-2 font-semibold">Property Accessible</h3>
					<p>{request.is_accessible ? "Yes" : "No"}</p>
				</div>
