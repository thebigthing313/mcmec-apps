import { WaterManagementRequestsInsertSchema } from "@mcmec/supabase/db/water-management-requests";
...
export const submitWaterManagementRequestServerFn = createServerFn({
	method: "POST",
})
	.inputValidator(WaterManagementRequestSchema)
	.handler(async ({ data }) => {
		...
		if (error) {
			console.error("Error inserting water management request:", error);
