import { MosquitofishRequestsInsertSchema } from "@mcmec/supabase/db/mosquitofish-requests";
...
export const submitMosquitofishRequestServerFn = createServerFn({
	method: "POST",
})
	.inputValidator(MosquitofishRequestSchema)
	.handler(async ({ data }) => {
		...
		if (error) {
			console.error("Error inserting mosquitofish request:", error);
