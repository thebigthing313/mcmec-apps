...
function RouteComponent() {
	...
	const handleSubmit = async (value: AdultMosquitoRequestsBaseType) => {
		const tx = db.adultMosquitoRequests.update(requestId, (draft) => {
	...
	const handleDelete = async () => {
		const tx = db.adultMosquitoRequests.delete(requestId);
		...
		<div className="space-y-4">
			...
			<AdultMosquitoForm
				defaultValues={{
					...
					is_dusk_dawn: request.is_dusk_dawn,
					is_front_of_property: request.is_front_of_property,
					is_general_vicinity: request.is_general_vicinity,
					is_nighttime: request.is_nighttime,
					is_processed: request.is_processed,
					is_rear_of_property: request.is_rear_of_property,
					phone: request.phone,
					updated_at: new Date(),
					updated_by: null,
					zip_code_id: request.zip_code_id,
				}}
				formLabel="Edit Adult Mosquito Complaint"
